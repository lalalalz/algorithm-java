...
public class 혼자_놀기의_달인 {
    public int solution(int[] cards) {
        ...
        for (int i = 0; i < cards.length; i++) {
            int aGroupBoxCount = openBox(i, cards, isOpen);
            ...
            for (int j = 0; j < cards.length; j++) {
                ...
                int bGroupBoxCount = openBox(j, cards, isOpen);
                max = Math.max(max, aGroupBoxCount * bGroupBoxCount);
                closeBox(j, cards, isOpen);
            }

            closeBox(i, cards, isOpen);
        }

        return max;
    }

    private void closeBox(int i, int[] box, boolean[] isOpen) {
        while (isOpen[i]) {
            isOpen[i] = false;
            i = box[i] - 1;
        }
    ...
    private int openBox(int i, int[] box, boolean[] isOpen) {
        int opendBoxCount = 0;
